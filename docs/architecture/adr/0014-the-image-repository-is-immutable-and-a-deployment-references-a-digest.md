...
None of it was written down here. No workflow logs into a registry; the `Makefile` carries the kubebuilder scaffold's `docker-build`, `docker-push` and `docker-buildx` at `IMG ?= controller:latest` with nothing resolving that default; and `grep -rniE 'ecr|garam-agent-operator-dev|registry' docs/ README.md` returned no hit about publishing. The only written record was another project's, and it names this repository only to exclude it: `sherlock@04ed05a:docs/architecture/adr/0020-immutable-image-repositories.md` says `garam/garam-agent-operator-dev` "shares the namespace and is the `garam-agent-operator` project's, not this one's."
...
- **This project has no build-time check that a tag tells the truth.** `sherlock` carries `REVISION` into `org.opencontainers.image.revision` in both its Dockerfiles and refuses a default-`REVISION` build whose context differs from the commit. `Dockerfile` here writes no label and no target compares anything against a commit. A tag in `sherlock`'s repositories is a claim a tool declined to let go wrong by default; a tag in this one is a claim a person typed.
...
One thing makes the argument stronger here than where it was made. `sherlock`'s tag is checked against the tree at build time and this project's is not, so the claim on offer here is the weaker one. An argument for not resting on a tag is not weakened by the tag being worth less.
...
- **The tag stays an unchecked claim.** Deciding the form does not add the guard `sherlock` has, and this does not add one — that is a change to `Dockerfile` and the `Makefile`, which #102 excludes. What bounds the damage is immutability and the fact that the digest, not the tag, is what runs.
...
- **Tag only, as `sherlock`'s own tags would allow.** Refused on the ground ADR 0020 gives and on one of this project's own: there is no build-time check here, so the tag is a weaker claim than the one that argument was already refused for.
